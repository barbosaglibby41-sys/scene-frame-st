...
function settingsRoot() { return extension_settings || globalThis.extension_settings || globalThis.SillyTavern?.getContext?.()?.extensionSettings || {}; }
function readSavedSettings() {
  ...
  // 优先读取酒馆的 extension_settings；仅首次用 legacy localStorage 填补空字段。
  const merged = { ...defaults, ...legacy, ...current };
  root[SETTINGS_KEY] = merged;
  try { saveSettingsDebounced(); } catch { try { globalThis.saveSettingsDebounced?.(); } catch {} }
  return merged;
}
const state = { settings: readSavedSettings(), seen: new Set(), current: null, pending: [], tasks: [], fabDragged: false, page: 'generate', panelOpen: false, panelScrollTop: 0, parserPreview: '', scanTimer: null, lastChatKey: '' };
function save() {
  const root = settingsRoot(); root[SETTINGS_KEY] = state.settings;
  try { saveSettingsDebounced(); } catch { try { globalThis.saveSettingsDebounced?.(); } catch {} }
...
function render() {
  ...
  root.querySelector('#sf-url')?.addEventListener('input', e => { if (state.settings.backend === 'nai') state.settings.naiUrl = e.target.value; else state.settings.sdUrl = e.target.value; save(); });
  root.querySelector('#sf-key')?.addEventListener('input', e => { state.settings.naiKey = e.target.value; save(); });
  root.querySelector('#sf-prefix')?.addEventListener('input', e => { state.settings.positivePrefix = e.target.value; state.settings.activePresetId = ''; save(); });
  root.querySelector('#sf-negative')?.addEventListener('input', e => { state.settings.negative = e.target.value; state.settings.activePresetId = ''; save(); });
  root.querySelectorAll('#sf-nai-model,#sf-nai-sampler,#sf-nai-width,#sf-nai-height,#sf-nai-steps,#sf-nai-scale,#sf-nai-seed,#sf-nai-samples,#sf-nai-ucpreset,#sf-nai-quality,#sf-nai-smea,#sf-nai-smeadyn').forEach(input => input.addEventListener('input', saveAllSettings));
  ...
  root.querySelector('#sf-danbooru-login')?.addEventListener('input', e => { state.settings.danbooruLogin = e.target.value.trim(); save(); });
  root.querySelector('#sf-danbooru-key')?.addEventListener('input', e => { state.settings.danbooruKey = e.target.value.trim(); save(); });
...
function scheduleChatScan(delay = 350) {
  clearTimeout(state.scanTimer); state.scanTimer = setTimeout(() => scanChatForImageBlocks().catch(error => console.debug('[SceneFrame] auto scan error', error)), delay);
}
...
function watchChatChanges() {
  ...
  const observer = new MutationObserver(() => {
    ...
    if (chatChanged || nodes !== lastNodeCount) { state.lastChatKey = key; lastNodeCount = nodes; scheduleChatScan(chatChanged ? 650 : 220); }
  });
...
function boot() {
  try {
    ...
    if (source?.on) {
      for (const key of ['MESSAGE_RECEIVED', 'MESSAGE_EDITED', 'MESSAGE_UPDATED']) { const type = types[key]; if (type) source.on(type, id => { inspectChatMessageById(id); scheduleChatScan(260); }); }
      for (const key of ['GENERATION_ENDED', 'MESSAGE_SWIPED', 'CHAT_CHANGED', 'CHAT_LOADED']) { const type = types[key]; if (type) source.on(type, () => scheduleChatScan(500)); }
    }
    state.unsubscribe = subscribeToMessages(message => { inspectMessage(message).catch(error => console.debug('[SceneFrame] fallback inspect error', error)); scheduleChatScan(260); });
    state.lastChatKey = currentChatKey(); watchChatChanges(); scheduleChatScan(700);
