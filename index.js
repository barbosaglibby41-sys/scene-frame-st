...
const state = { settings: { ...defaults }, seen: new Set(), current: null, pending: [], tasks: [], fabDragged: false, page: 'generate', panelOpen: false, panelScrollTop: 0, parserPreview: '' };
...
const queue = new TaskQueue({ onChange: tasks => {
  state.tasks = tasks;
  for (const item of tasks) if (item.messageId != null) renderGenerateAction({ messageId: item.messageId, item, onGenerate: enqueueItem, onDismiss: dismissItem });
...
function addPending(item) { state.pending = [...state.pending.filter(x => x.id !== item.id), item]; state.current = item; renderPending(); if (item.messageId != null) renderGenerateAction({ messageId: item.messageId, item, onGenerate: enqueueItem, onDismiss: dismissItem }); }
...
async function scanChatForImageBlocks() {
  const context = getTavernContext(); const messages = context?.chat || window.chat || [];
  let count = 0;
  for (let id = 0; id < messages.length; id++) {
    ...
    for (const block of extractByRules(text, state.settings.parserRules)) {
      ...
      if (stored && !document.querySelector(`.sf-inline-action[data-sf-id="${stored.id}"]`)) renderGenerateAction({ messageId: id, item: { ...stored, status: 'pending' }, onGenerate: enqueueItem, onDismiss: dismissItem });
    }
  }
  return count;
}
