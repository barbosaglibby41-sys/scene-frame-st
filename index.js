...
function cardMarkup(item) {
  ...
  return `<div class="sf-wrap-head"><span>\u{1F5BC}\uFE0F \u56FE\u7247\u751F\u6210\u8BF7\u6C42</span><span class="sf-wrap-rule">${item.ruleName || "\u56FE\u7247\u5757"}</span></div><div class="sf-wrap-hint">\u957F\u6309\u201C\u751F\u6210\u56FE\u7247\u201D\u67E5\u770B\u539F\u59CB Tag</div><div class="sf-wrap-actions"><button class="sf-inline-generate ${status === "generating" || status === "queued" ? "sf-loading" : ""}" ${disabled ? "disabled" : ""}>${status === "generating" || status === "queued" ? '<span class="sf-spinner"></span>' : "\u{1F5BC}\uFE0F"} ${label}</button>${status === "pending" ? '<button class="sf-inline-dismiss" title="\u5FFD\u7565">\xD7</button>' : ""}</div>${status === "failed" ? `<div class="sf-inline-error">\u751F\u6210\u5931\u8D25\uFF1A${esc(item.error || "\u8BF7\u68C0\u67E5 API \u8BBE\u7F6E")}</div>` : ""}<details class="sf-wrap-detail"><summary>\u67E5\u770B\u89E3\u6790\u6807\u7B7E</summary><pre></pre></details>`;
}
...
function bindCard(card, item, onGenerate, onDismiss) {
  ...
  card.querySelector(".sf-inline-dismiss")?.addEventListener("click", () => {
    card.replaceWith(document.createTextNode(item.matched || ""));
    onDismiss?.(item);
  });
}
function renderGenerateAction({ messageId, item, onGenerate, onDismiss }) {
  ...
  bindCard(card, item, onGenerate, onDismiss);
  return true;
}
...
var state = { settings: readSavedSettings(), seen: /* @__PURE__ */ new Set(), current: null, pending: [], tasks: [], fabDragged: false, page: "generate", panelOpen: false, panelScrollTop: 0, parserPreview: "" };
function save() {
  ...
  try {
    ...
    if (source?.on) {
      for (const key of ["MESSAGE_RECEIVED", "MESSAGE_EDITED", "MESSAGE_UPDATED"]) {
        ...
        if (type) source.on(type, inspectChatMessageById);
      }
      for (const key of ["GENERATION_ENDED", "MESSAGE_SWIPED"]) {
        const type = types[key];
        if (type) source.on(type, () => setTimeout(() => scanChatForImageBlocks().catch(() => {
        }), 280));
      }
    }
