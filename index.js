...
async function scanChatForImageBlocks() {
  const context = getTavernContext(); const messages = chat || context?.chat || window.chat || [];
  let count = 0;
  for (let id = 0; id < messages.length; id++) {
    ...
    for (const block of extractByRules(text, state.settings.parserRules)) {
      ...
      if (stored && !document.querySelector(`.sf-inline-action[data-sf-id="${stored.id}"]`)) scheduleInlineAction({ ...stored, status: 'pending' });
    }
  }
...
function inspectChatMessageById(id) {
  const messageId = Number(id); const message = chat?.[messageId];
...
function boot() {
  render();
  // 直接使用 SillyTavern/TauriTavern 官方扩展事件；与参考插件的消息监听方式一致。
  for (const key of ['MESSAGE_RECEIVED', 'MESSAGE_EDITED', 'MESSAGE_UPDATED']) {
    const type = event_types?.[key]; if (type) eventSource.on(type, inspectChatMessageById);
  }
  for (const key of ['GENERATION_ENDED', 'MESSAGE_SWIPED']) {
    const type = event_types?.[key]; if (type) eventSource.on(type, () => setTimeout(() => scanChatForImageBlocks().catch(() => {}), 280));
  }
  ...
  console.info('[SceneFrame] loaded with native SillyTavern events', diagnostics());
}
